...
def get_classement():
    ...
    '''
    classement = []
    while not os.path.isfile('stats.py'):
        file = open("stats.py", "w")
        file.close()
    fichier = open('stats.py', 'r')
    for ligne in fichier:
        if not ligne == "\n":
            classement.append(ligne)
            print("[utils.elo_utilities] get_classement:" + classement[-1])
    fichier.close()
    i = 0
...
def get_elo(target):
    ...
    for i in range(len(classement)):
        if str(classement[i][0]) == str(target): 
            ...
            print("[utils.elo_utilities] get_elo: " + target + ": " + str(elo_target) + "Elo")
            return elo_target
    
    fichier = open('stats.py', 'a')
    fichier.write(target + ' 1000 0 0 0 0\n')
    print("[utils.elo_utilities] get_elo: " + "[New Player added] " + target)
    fichier.close()
    return 1000
    #==================================================
...
def get_game(target):
    ...
    for i in range(len(classement)):
        if str(classement[i][0]) == str(target): 
            ...
            print("[utils.elo_utilities] get_game: " + target + ": " + str(game) + " games")
            return game
    return 0
    #==================================================
    ...
    if target == "personne":
        ...
        for i in range(len(classement)):
            if str(classement[i][0]) == str(target): 
                ...
                print("[utils.elo_utilities] get_win: " + target + ": " + str(win) + " victoires")
                return win
    #==================================================
...
def get_winstreak(target):
    ...
    for i in range(len(classement)):
        if str(classement[i][0]) == str(target): 
            ...
            print("[utils.elo_utilities] get_winstreak: " + target + ": " + str(winstreak) + " victoires d'affilee")
            return winstreak
    #==================================================
...
def get_goal(target):
    ...
    for i in range(len(classement)):
        if str(classement[i][0]) == str(target): 
            ...
            print("[utils.elo_utilities] get_goal: " + target + ": " + str(goal) + " but")
            return goal
    #==================================================
...
def set_elo(target, elo):
    ...
    for i in range(len(classement)):
        if str(classement[i][0]) == str(target): 
            ...
            fichier = open('stats.py', 'w')
            j = 0
            for j in range(len(classement)):
                print("[utils.elo_utilities] set_elo:  " + str(classement[j]))
...
def set_game(target):
    ...
    for i in range(len(classement)):
        if str(classement[i][0]) == str(target): 
            ...
            j = 0
            for j in range(len(classement)):
                print("[utils.elo_utilities] set_game: " + str(classement[j]))
    ...
    if score == 1:
        ...
        for i in range(len(classement)):
            if str(classement[i][0]) == str(target): 
                ...
                j = 0
                for j in range(len(classement)):
                    print("[utils.elo_utilities] set_win: " + str(classement[j]))
                    fichier.write(str(classement[j][0] + " " + str(classement[j][1]) + " " + str(classement[j][2]) + " " + str(classement[j][3]) + " " + str(classement[j][4]) + " " + str(classement[j][5]) + "\n"))
                fichier.close()
    return   
    #==================================================
        ...
        if winstreak <= 0:
            ...
            for i in range(len(classement)):
                if str(classement[i][0]) == str(target): 
                    ...
                    for j in range(len(classement)):
                        print("[utils.elo_utilities] set_winstreak: " + str(classement[j]))
...
def set_goal(target, score):
    ...
    for i in range(len(classement)):
        if str(classement[i][0]) == str(target): 
            ...
            j = 0
            for j in range(len(classement)):     
                print("[utils.elo_utilities] set_goal: " + str(classement[j]))
                fichier.write(str(classement[j][0] + " " + str(classement[j][1]) + " " + str(classement[j][2]) + " " + str(classement[j][3]) + " " + str(classement[j][4]) + " " + str(classement[j][5]) + "\n"))
            fichier.close()
    return   
    #==================================================
...
def get_delta_elo(target):
    ...
    if target == "random":
        return 0
    delta_elo = []
    while not os.path.isfile('cache.py'):
        file = open("cache.py", "w")
        file.close()  
    fichier = open('cache.py', 'r')
    for ligne in fichier:
        if not ligne == "\n":
            delta_elo.append(ligne)
            print("[utils.elo_utilities] get_delta_elo: " + delta_elo[-1])
    fichier.close()
    i = 0
    ...
    for j in range(len(delta_elo)):
        if str(delta_elo[i][0]) == str(target): 
            delta_elo_target = int(delta_elo[i][1])
            print("[utils.elo_utilities] get_delta_elo: " + target + ": " + str(delta_elo_target) + "Elo")
            return delta_elo_target, delta_elo
    fichier = open("cache.py","a")
    ...
    if delta_elo_target < 50:
        ...
        for i in range(len(delta_elo)):
            if str(delta_elo[i][0]) == str(target): 
                ...
                fichier = open('cache.py', 'w')
                j = 0
                for j in range(len(delta_elo)):
                    delta_elo[j][1] = str(delta_elo[j][1])
                    print("[utils.elo_utilities] delta_elo: " + str(delta_elo[j]))
