...
def processing_match():
    ...
    senders = []
    contenu_mail = []
    results = search_messages(service, "Subject:CLASSEMENT in:inbox")  
    for msg in results:
    ...
    for i in range(len(contenu_mail)):
        ...
        if match_list[-1] == '':
            match_list.remove('')
        if len(match_list) == 4:
            A = match_list[0].split(" ")
            B = match_list[1].split(" ")
            C = match_list[2].split(" ")
            D = match_list[3].split(" ")
            
            if len(A) != 2 or len(B) != 2 or len(C) != 2 or len(D) != 2:
                send_message(service, senders[i], "Elo Rating SMC Program", "Format invalide pour l'email\n" + str(match_list) + '\n\n Trop ou pas assez d\'argument sur une ligne')
                print("[processing_match] " + "pas assez d'argument sur une ligne'")
