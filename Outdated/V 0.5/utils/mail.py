...
def get_receiver():
    ...
    '''
    receiver = []
    while not os.path.isfile('receiver.py'):
        file = open("receiver.py", "a")
        file.close()
    fichier = open('receiver.py', 'r')
    for ligne in fichier:
        if not ligne == "\n":
            receiver.append(ligne)
            print("[utils.mail] get_receiver: " + receiver[-1])
    fichier.close()
    i = 0
...
def set_receiver(target, state):
    ...
    if state == True:
        ...
        try:
            receiver = receiver.remove(target)
        except ValueError:
            pass
        i = 0
        fichier = open('receiver.py', 'w')
        for i in range(len(receiver)):
            fichier.write(receiver[i] + "\n")
        fichier.write(target)
        fichier.close()
        print("[utils.mail] set_receiver: " + target + " " + str(state))
    elif state == False:
        i = 0
        for i in range(len(receiver)):
            if str(receiver[i][0]) == str(target): 
                try:
