...
def elo_update():
    ...
    '''
    service = service_drive() #getting drive service
    ...
    file.close()
    upload_files(service,filename, classement_folder) #uploading file then we'll delete it
    ...
    for i in range(len(classement)):
        folder_id = search_for_folder(service,classement[i][0])
